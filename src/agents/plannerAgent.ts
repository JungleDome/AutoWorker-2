...
export async function runPlannerForTicket(
  ...
  if (envelope.payload_type !== "plan") {
    throw new Error(
      `Planner must return payload_type="plan", got "${envelope.payload_type}"`,
    );
  }
  if (envelope.agent_role !== PLANNER_ROLE) {
    throw new Error(
      `Planner must return agent_role="${PLANNER_ROLE}", got "${envelope.agent_role}"`,
    );
  }
  if (envelope.ticket_id !== options.ticketId) {
    throw new Error(
      `Planner returned ticket_id="${envelope.ticket_id}" but expected "${options.ticketId}"`,
    );
  }
...
function buildPlannerPrompt(options: PlannerRunOptions): string {
  ...
  if (Array.isArray(req.acceptance_criteria)) {
    for (const acRaw of req.acceptance_criteria as unknown[]) {
      ...
      const mustHave =
        typeof ac.must_have === "boolean" ? ac.must_have : true;
  ...
  const constraintsSection =
    constraintsArray.length > 0
      ? [
          "Constraints:",
          ...constraintsArray.map((item) => `- ${item}`),
          "",
        ]
      : [];
  ...
  const dependenciesSection =
    dependenciesArray.length > 0
      ? [
          "Dependencies (other tickets/work items):",
  ...
  const notesFromRequirementsSection = freeformNotes
    ? [
        "Additional notes from requirements.freeform_notes:",
        freeformNotes,
        "",
      ]
    : [];
  ...
  const humanNotesSection =
    options.notesForAgent && options.notesForAgent.trim().length > 0
      ? [
          "Additional notes or feedback from controller/human:",
